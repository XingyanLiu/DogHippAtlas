...
def normalize_reverse(adata, counts_level=None, frac=0.05, setraw=True,
                      copy=False):
...
def partialUMAPAdt(adata, based_on, n_neighbors=20,
    ...
    labels = adata.obs[based_on[0]]
    train_on = based_on[1]
    print('Computing partial-UMAP on class `{}` grouped by `{}`'.format(
        *based_on))
...
def GroupZscoreAdt(adt, key='counts', groupby='batch', key_new=None,
    ...
    if key == 'counts':
        print(
            'doing z-score scaling on count matrix, transformed into a dense array')
    ...
    #        print('TEST:, adt.X.shape)
        ...
        adt.obsm[key_new] = GroupZscore(adt.obsm[key], labels, \
                                        with_mean=with_mean, max_value=None,
                                        **kwds)
...
def label_mask_others(df, key, keeps, key_new='tmp', lb_masked='others',
                      copy=False):
...
def MatchByMarkersAdt(adata, markers=None, ntop=10):
    ...
    '''
    if markers is None:
        markers = pd.DataFrame(adata.uns['rank_genes_groups']['names']).head(
            ntop)
...
def GroupSimilarityMean(adatas, keys, use_genes=None,
    ...
    * clusters from different adatas
    ...
    if isinstance(adatas, sc.AnnData):  # single case
        ...
        gmeans = GroupMean(adatas, groupby=key, features=use_genes,
                           use_raw=use_raw)
        ...
        gmeans = [GroupMean(adt, ky, use_genes, use_raw=use_raw) for adt, ky in
                  zip(adatas, keys)]
...
def save_embeddings(adata, path, key=None, ndim=2, tail='', ):
    ...
    pd.DataFrame(adata.obsm[key][:, : ndim]).to_csv(path, header=False,
                                                    index=False)
...
def ResetX(adata, copy=True):
    ...
    print(
        'Replacing the scaled matrix by the count data. Dimension of columns (genes) are not changed.')
    print(_adata)
